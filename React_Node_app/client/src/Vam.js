...
function Vam(props) {
  ...
  const [vaseries,setvaseries] = useState([ { name: 'VAM', data: [] }])

 const getChartData = () => {
   console.log("props.productionUnitData-->",props.productionUnitData)
   ...
   for (let i=0; i < datas.length;i++){
     ...
     seriesdata.push(parseFloat(datas[i].VAM).toFixed(1));
  ...
  let updateseries = { name: 'VAM', data: [...seriesdata] };
...
<div id="chart" className="haciichart">
 ...
 <div>
  <div className="left-float">Vam<p></p></div>  
 
  <div className="right-float"> <p>O2 Feed</p>
       <p> { vaseries[0].data.length ? parseFloat(vaseries[0].data[vaseries[0].data.length - 1]).toFixed() : ''} LB/HR</p>
  </div>
</div>
</div>
